...
static void libtw_error(uldat err, uldat err_detail) {
  fprintf(stderr, "twplayer_remote: libTw error: %s%s\n", TwStrError(err),
          TwStrErrorDetail(err, err_detail));
}
...
static void update_window_title(void) {
  ...
  if (pl && pl->label) {
    ...
    if (len) memcpy(title + 9, pl->label, len);
    len += 9;
  }
...
static void clear_win_remote(void) {
  ...
  for (i = 0, n = factories->size; i < n; i++) {
    ...
    if (menuitem != TW_NOID) TwDeleteMenuItem(menuitem);
  }
  TwFlush();
}
...
static void fill_win_remote(void) {
  ...
  for (i = 0, n = factories->size; i < n; i++) {
    ...
    if (factory->label != NULL) {
      ...
      TwRow4Menu(win_remote, G_REMOTE_FIRST + i, TW_ROW_ACTIVE, len + 2, label);
    }
  }
}
  ...
  if (pl && *track) {
    if (trackXPosition > 0) {
      if ((len = 20 - trackXPosition) > 0) {
        if (len > trackLen) len = trackLen;
        memcpy(buf + trackXPosition, track, len);
      }
    } else {
      if ((len = trackLen + trackXPosition) > 20) len = 20;
...
static void paint(void) {
  ...
  if (pl) {
    ...
    sprintf(buf, "\x11%02d:%02d/%02d:%02d\x10 \x1F%03ld%%\x1E", cTime / 60,
            cTime % 60, tTime / 60, tTime % 60, volume);
  } else {
    memset(buf, ' ', 20);
  }
  ...
  if (trackLen < myLen) {
    ...
    if (trackScrollMode == 0) {
      ...
      if (trackScrollSpeed > 0) {
        if (trackXPosition < -trackLen) trackXPosition = myLen;
      } else {
        if (trackXPosition > myLen) trackXPosition = -trackLen;
      }
      ...
      if (trackScrollSpeed > 0) {
        ...
        if (trackXPosition > 15) trackScrollSpeed = -trackScrollSpeed;
      }
    }
  }
}
...
void load_track_info(void) {
  ...
  size_t len, len2;
  if (pl) pl->get_trackinfo(pl, &info);

  len = info.artist_len;
  len2 = info.title_len;

  if (len) {
    if (len >= sizeof(track) / 2) len = sizeof(track) / 2 - 1;

    if (len) memcpy(track, info.artist, len);
  }
  if (len && len2) {
    memcpy(track + len, " - ", 3);
    len += 3;
  ...
  if (len2) {
    if (len2 >= sizeof(track) - len) len2 = sizeof(track) - len - 1;
  ...
  if (!len) memcpy(track, "<<unknown>>", (len = 11));
  ...
  if (res != NULL) {
    ...
    if (xmmsc_result_get_class(res) == XMMSC_RESULT_CLASS_DEFAULT) {
      ...
      if (val != NULL) {
        val =
            xmmsv_propdict_to_dict(val, NULL);  // must call xmmsv_unref() later
        ...
        if (info_artist_len) {
          ...
          if (info_artist_len) memcpy(title, info_artist, info_artist_len);
        }
...
static void event_menu(tevent_menu event) {
  ...
  switch (code) {
    ...
    case G_RESCAN:
      ...
      break;
    default:
      if (code >= G_REMOTE_FIRST && factories &&
          code - G_REMOTE_FIRST < factories->size) {
        disconnect_remote();
...
static void add_timeval(struct timeval *t1, struct timeval *t2,
                        struct timeval *t3) {
  *t1 = *t2;
  inc_timeval(t1, t3);
}

static void sub_timeval(struct timeval *t1, struct timeval *t2,
                        struct timeval *t3) {
  *t1 = *t2;
  dec_timeval(t1, t3);
}
...
static void main_loop(void) {
  ...
  if ((err = TwErrno)) libtw_error(err, TwErrnoDetail);
}
