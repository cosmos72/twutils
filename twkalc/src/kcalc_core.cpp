/*
    $Id: kcalc_core.cpp,v 1.21 1999/02/03 10:23:06 radej Exp $
...
void QtCalculator::InitializeCalculator(void) {
  ...
  // up the floating point exception signal handler to trap the
...
void QtCalculator::EnterDigit(int data)
{
  ...
  if (eestate){
	  ...
	  buf[0] = (char)data - 10 + 'A';
      strcat(display_str,buf);
...
void QtCalculator::EnterDecimal()
{
  ...
  if (last_input == DIGIT && !strpbrk( display_str,".")){
    ...
    calc_display->setText(strcat(display_str, "."), -1);
  }
  else {
    ...
    calc_display->setText(display_str, -1);
  }
}
...
void QtCalculator::EnterNegate()
{
  ...
  if(eestate){
    int pos;
    char *ppos = strrchr(display_str, 'e');
    if (!ppos)
      return;
    pos = ppos - display_str;
	...
	memmove(display_str+pos+2, display_str+pos+1, strlen(display_str+pos));
	display_str[pos+1] = '-';
      }
    }
    DISPLAY_AMOUNT = (CALCAMNT)strtod(display_str,0);
...
void QtCalculator::ComputeSin()
{
  ...
  if (hyp_mode){
	...
	display_error = 1;
      inverse = tfalse;       // reset the inverse flag
    }
  }
  else {  
    // sine or arcsine
...
int UpdateStack(int run_precedence)
{
	...
	if ((top_item = PopStack())->s_item_type != ITEM_AMOUNT){
	    TDialog::message( "Error", 
			     "Stack processing error - right_op", "OK" );
	...
	if (!((top_item = PopStack()) && 
	    ...
	    TDialog::message( "Error", 
			     "Stack processing error - function", "OK" );
	...
	if (!((top_item = PopStack()) && 
	    ...
	    TDialog::message( "Error", 
			     "Stack processing error - left_op", "OK" );
	}
...
stack_ptr AllocStackItem (void) {
    ...
    TDialog::message( "Emergency", "Stack Error!", "OK" );
    return &process_stack[stack_next];
}
