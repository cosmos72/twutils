...
static int mywidth, volume, trackTime, currentTime, playlistpos,
    displayElapsedTime;
...
static void libxmms_error(int err) {
  fprintf(stderr, "twxmms2: libxmmsclient error %d: %s\n", err,
          conn ? xmmsc_get_last_error(conn) : "xmmsc_init() failed");
}
  ...
  if (connected && *title) {
    if (titleXPosition > 0) {
      if ((len = 20 - titleXPosition) > 0) {
        if (len > titleWidth) len = titleWidth;
        memcpy(buf + titleXPosition, title, len);
      }
    } else {
      if ((len = titleWidth + titleXPosition) > 20) len = 20;
...
static void paint(void) {
  ...
  if (connected) {
    ...
    sprintf(buf, "\x11%02d:%02d/%02d:%02d\x10 \x1F%03d%%\x1E", cTime / 60,
  ...
  if (titleWidth < mywidth) {
    ...
    if (titleScrollMode == 0) {
      ...
      if (titleScrollSpeed > 0) {
        if (titleXPosition < -titleWidth) titleXPosition = mywidth;
      } else {
        if (titleXPosition > mywidth) titleXPosition = -titleWidth;
      }
      ...
      if (titleScrollSpeed > 0) {
        ...
        if (titleXPosition > 15) titleScrollSpeed = -titleScrollSpeed;
      }
    }
  }
}
    ...
    if (xmmsc_result_get_class(res) == XMMSC_RESULT_CLASS_DEFAULT) {
      ...
      if (val != NULL) {
        ...
        if (track_artist_len) {
          ...
          if (track_artist_len) memcpy(title, track_artist, track_artist_len);
        }
...
static int startXmms(void) {
  ...
  switch (pid) {
    ...
    default:
#ifdef HAVE_WAITPID
      if (waitpid(pid, &wstatus, 0) != pid) wstatus = -1;
#else
  ...
  if (conn || (conn = xmmsc_init("twxmms2"))) {
    if (connected) {
      ...
      if (err != 0) {
        ...
        if ((err = startXmms()) == 0) err = !xmmsc_connect(conn, server_name);
      }
    }
  }
  connected = err == 0;
...
static void event_gadget(tevent_gadget event) {
  ...
  if (!connected && connectXmms() != 0) return;
...
static void event_mouse(tevent_mouse event) {
  ...
  if (event->Code == PRESS_LEFT && event->W == xmms_Win && event->Y == 1) {
    ...
    if (event->X <= 12) {
      if (event->X <= 5) {
        if ((currentTime -= trackTime / 20) < 0) currentTime = 0;
      } else {
...
int main(int argc, char **argv) {
  if (init_gui(argc, argv)) main_loop();
